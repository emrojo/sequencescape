  ...
  module TooltipHelper
    ...
    def render_tooltip(name, content, opts)
      content_tag(
        ...
        content_tag(
          :div,
          content << content_tag(:small, close_tooltip_link(opts[:id], opts[:close_message])),
          :id => "tooltip_content_#{opts[:id]}", :class => 'tooltip_content'
        ),
        :id => "tooltip_#{opts[:id]}", :class => 'tooltip', :style => 'display:none'
      )
    end
  end
end
