...
class LabEvent < ActiveRecord::Base
 ...
 scope :with_descriptor, lambda { |k,v| { :conditions => [ 'descriptors LIKE ?', "%#{k.to_s}: #{v.to_s}%" ] } }

 scope :barcode_code, lambda { |*args| {:conditions => ["(description = 'Cluster generation' or description = 'Add flowcell chip barcode') and eventful_type = 'Request' and descriptors like ? ", args[0]] }}
