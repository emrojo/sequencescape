...
class BroadcastEvent < ActiveRecord::Base
  ...
  module ClassMethods
    ...
    def has_subject(role_type,method=nil,&block)
      return subject_associations << Helpers::SimpleSingleSubjectAssociation.new(role_type,method) unless method.nil?
      return subject_associations << Helpers::BlockSingleSubjectAssociation.new(role_type,&block) unless block.nil?
    ...
    def has_subjects(role_type,method=nil,&block)
      return subject_associations << Helpers::SimpleManySubjectAssociation.new(role_type,method) unless method.nil?
      return subject_associations << Helpers::BlockManySubjectAssociation.new(role_type,&block) unless block.nil?
      raise StandardError, "No block or method defined for #{role_type} on #{name}"
    end
