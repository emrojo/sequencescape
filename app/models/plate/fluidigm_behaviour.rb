...
module Plate::FluidigmBehaviour
  ...
  def self.included(base)
    base.class_eval do
      ...
      scope :requiring_fluidigm_data, lambda {
        ...
        joins([
          ...
          "INNER JOIN requests ON requests.target_asset_id = fluidigm_plate_association.content_id AND state = \'passed\' AND requests.request_type_id = #{RequestType.find_by_key!('pick_to_fluidigm').id}", # Link to their requests
