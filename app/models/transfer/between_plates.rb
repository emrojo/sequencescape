...
class Transfer::BetweenPlates < Transfer
  ...
  def each_transfer(&block)
    r=request_type_between(source, destination)
    ...
    bad_wells, good_wells = source.wells.located_at_position(transfers.keys).with_pool_id.partition {|well| r.request_class.should_well_not_be_transferred?(well)}
